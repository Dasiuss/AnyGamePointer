...
function showGame() {
  ...
  app.innerHTML = `<section class="screen game-screen">
    <header class="screen-header"><div><h1>Wyniki</h1><p class="subtitle">Kliknij numer rundy, aby zmienić punktację.</p></div><button class="secondary" data-action="new-game">Nowa gra</button></header>
    <div class="table-wrap"><table class="score-table"><thead><tr><th>Runda</th>${players.map(player => `<th><span class="player-name">${escapeHtml(player.name)}</span><strong class="total">${totals[player.id]}</strong></th>`).join('')}</tr></thead><tbody>
      ${state.game.rounds.map((round, index) => `<tr class="round-row ${selected === index ? 'selected' : ''}"><td rowspan="${selected === index ? 2 : 1}"><div class="round-number"><button data-action="select-round" data-round="${index}">${index + 1}</button>${index === state.game.rounds.length - 1 ? '<button data-action="add-round" aria-label="Dodaj rundę">+</button>' : ''}</div></td>${players.map(player => `<td><span class="round-score">${scoreFor(round, player.id)}</span></td>`).join('')}</tr>${selected === index ? `<tr class="controls-row">${players.map(player => `<td><div class="score-buttons">${[1,2,3,4,5,6,7].map(value => `<button data-action="set-score" data-player="${player.id}" data-score="${value}">${value}</button>`).join('')}</div><button class="plus-button" data-action="open-keypad" data-player="${player.id}" aria-label="Wpisz większą liczbę punktów">+</button></td>`).join('')}</tr>` : ''}`).join('')}
    </tbody></table></div>
  </section>`;
}
...
keypad.addEventListener('click', event => { if (event.target === keypad) return closeKeypad(); const key = event.target.closest('[data-key]')?.dataset.key; if (!key) return; if (key === 'backspace') keypadDraft = keypadDraft.slice(0, -1); else if (key === 'ok') { updateScore(keypadTarget, keypadDraft === '' ? 0 : Number(keypadDraft)); return closeKeypad(); } else keypadDraft += key; keypadValue.textContent = keypadDraft || '0'; });
