...
function showGame() {
  ...
  app.innerHTML = `<section class="screen game-screen">
    <div class="table-wrap"><table class="score-table"><thead><tr><th>#</th>${players.map(player => `<th><span class="player-name">${escapeHtml(player.name)}</span><strong class="total">${totals[player.id]}</strong></th>`).join('')}</tr></thead><tbody>
